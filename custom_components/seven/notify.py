"""telnyx SMS & text-to-speech calling for notify component."""
...
BASE_API_URL = "https://api.telnyx.com/v2/"
TIMEOUT = 5
TYPE_SMS = "messages"
...
def get_service(hass, config, discovery_info=None):
    """Get the telnyx notification service."""
    if not _authenticate(config):
        _LOGGER.error("You are not authorized to access telnyx")
        return None
    return TelnyxNotificationService(config)


class TelnyxNotificationService(BaseNotificationService):
    """Implementation of a notification service for the telnyx service."""
...
def _authenticate(config):
    ...
    return res.status_code == HTTPStatus.OK and "." in json.loads(res.text)['data']['balance']
